...
dsy.add("0_1",["北京",""]);
dsy.add("0_1_0",["海淀","朝阳","东城","西城","崇文","宣武","丰台","石景山","房山","门头沟","通州","顺义","昌平","密云县","怀柔","延庆县","平谷","燕郊"]);
...
dsy.add("0_4_1",["东莞市",""]);
dsy.add("0_4_2",["禅城","南海","顺德","山水","高明"]);
dsy.add("0_4_3",["荔湾","越秀","东山","天河","海珠","黄埔","芳村","白云","花都","番禺","从化","增城"]);
...
dsy.add("0_23",["上海",""]);
dsy.add("0_23_0",["闵行","徐汇","长宁","静安","卢湾","虹口","黄浦","浦东新","杨浦","闸北","普陀","嘉定","宝山","青浦","奉贤","南汇","崇明","金山","松江"]);
...
dsy.add("0_24_2",["青羊","锦江","金牛","武侯","成华","龙泉驿","青白江","温江","新都","高新","大邑","金堂","郫县","蒲江","双流","新津","崇州","都江堰","彭州","邛崃"]);
...
dsy.add("0_25",["天津",""]);
dsy.add("0_25_0",["和平","西青","北辰","大港","南开","河东","河西","河北","津南","红桥","塘沽","汉沽","东丽"]);
...
dsy.add("0_30",["重庆"]);
dsy.add("0_30_0",["南岸","渝北","万盛","大渡口","万州","北碚","沙坪坝","巴南","双桥","涪陵","江北","九龙坡","渝中","潼南","铜梁","长寿","合川"]);

var s=["memProvince","memCity","memArea"];
var opt0 = ["","",""];
var opt1 = ["请选择","请选择","请选择"];
function setup()
{
...
function user()
{
s=["receiveinfo_province","receiveinfo_city","receiveinfo_area"];
for(i=0;i<s.length-1;i++)
  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
change(0);
}
...
for(i=0;i<s.length-1;i++)
  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
data(0,province,city,area);
}

function data(v,province,city,area)
{
    var str="0";
    for(i=0;i<v;i++)
    { 
    ...
    {
        length = 0;
        options[0]=new Option(opt1[v],opt0[v]);
        if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v)
        {
            ...
            if(dsy.Exists(str))
            {
                ...
                for(i=0;i<ar.length;i++)
                {
                    ...
                    if (options[i].value == province && v==0)
                    {
                        options[i].selected = true;
                    }
                    if (options[i].value == city && v==1)
                    {
                        options[i].selected = true;
                    }
                    if (options[i].value == area && v==2)
                    {
                        options[i].selected = true;
                    }
                }
