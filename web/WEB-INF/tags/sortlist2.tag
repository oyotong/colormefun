...
<c:if test="${fn:length(i.sortMask) eq 3 }"><option style="color:red" ${value eq i.sortId?'selected="selected"':'' } value="${i.sortId }">${i.sortName }</option></c:if>
<c:if test="${fn:length(i.sortMask) eq 6 }"><option style="color:blue" ${value eq i.sortId?'selected="selected"':'' } value="${i.sortId }">&nbsp;&nbsp;${i.sortName }</option></c:if>
